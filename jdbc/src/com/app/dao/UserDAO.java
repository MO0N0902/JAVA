...
public class UserDAO {
	...
	public void join(UserDTO userDTO) {
		String query = "INSERT INTO TBL_USER(user_number, user_id, user_pw, "
				+ "user_name, user_age, user_gender, user_birth) " + "VALUES(SEQ_USER.NEXTVAL, ?, ?, ?, ?, ?, ?)";
	...
	public int login(String userId, String userPw) {
		...
		try {
			...
			if (resultSet.next()) {
				// resultSet의 초기 상태는 첫번째 행 이전의 커서가 위치함
				// next를 호출하면 커서를 다음 행으로 이동시키고 해당 행이 있으면 true를 반환함
				// 더 이상 행이 없다면 false를 반환
				userNumber = resultSet.getInt(1);
				// resultSet.getInt(int columnIndex) : 결과 집합에서 현재 커서가 위치한 행의 특정 열값을 정수로 가져오는 메소드
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public List<String> findId(String userName, String userBirth) {
	   ...
	   public UserDTO findUser(int userNumber) {
	      String query = "SELECT USER_ID, USER_PW, USER_NAME, "
	            + "USER_AGE, USER_GENDER, USER_BIRTH "
	            + "FROM TBL_USER "
	            + "WHERE USER_NUMBER = ?";
