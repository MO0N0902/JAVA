...
//
		...
		Scanner sc = new Scanner(System.in); // 1) 스캐너 입력 클래스 import
		int num = 0; // 2) 변수 num으로 저장
		System.out.println("구구단을 어디까지 출력할까요?"); // 2-1) 출력메세지"구구단을 어디까지 출력할까요?"
		num = sc.nextInt(); // 3) nextint 입력받은수 num에 저장
		for(int i = 1; i<=9; i++){				// 4) 외부 for(int i=1; i<=9; i++){} --> 구구단 9단까지 확실
			for(int j = 1; j<=num; j++) {	// 4-1) 내부 for(int j=1; j<=num; j++){} --> 입력받은 단까지. (변화)
				System.out.printf("%d * %d = %d\t", j, i ,j*1); // 4-2) 내부 for문 안에서 : printf("%d * %d = %d\t", num, i, num*i);
			}
			System.out.println(); // 4-3) 외부 for문 안에서 : 줄바꿈 println();
		}
